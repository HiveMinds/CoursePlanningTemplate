### Call this from another file, for hw 11, question 3b:
...
class Plot_to_tex:
    ...
    def plotSingleLine(self,x_path,y_series,x_axis_label,y_axis_label,label,filename,legendPosition,hw_nr):
        fig=plt.figure();
        ...
        plt.savefig('../../latex/hw'+str(hw_nr)+'/images/'+filename+'.png');
#         plt.show();

    # plot graphs
    def plotMultipleLines(self,x,y_series,x_label,y_label,label,filename,legendPosition,hw_nr):
        fig=plt.figure();
        ax=fig.add_subplot(111);
    ...
    def put_table_in_tex(self, table_matrix,filename,hw_nr):
        cols = np.shape(table_matrix)[1]
        format = "%s"
        for col in range(1,cols):
            format = format+" & %s"
        format = format+""
        np.savetxt("../../latex/hw"+str(hw_nr)+"/tables/"+filename+".txt",table_matrix, delimiter=' & ', fmt=format, newline='  \\\\ \hline \n')
    ...
    def example_create_a_table(self):
        ...
        self.put_table_in_tex(table_matrix,table_name,hw)
